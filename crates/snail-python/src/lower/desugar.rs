                ...
                parts
                    ...
                    .map(|part| match part {
                        ...
                        FStringPart::Expr(expr) => {
                            FStringPart::Expr(Box::new(self.desugar_expr(expr, prelude)))
                        }
                    })
                    .collect(),
            ),
...
fn expr_contains_complex_lambda(expr: &Expr) -> bool {
    match expr {
        ...
        Expr::FString { parts, .. } => parts.iter().any(|part| match part {
            FStringPart::Expr(expr) => expr_contains_complex_lambda(expr),
...
fn regex_pattern_contains_complex_lambda(pattern: &RegexPattern) -> bool {
    match pattern {
        ...
        RegexPattern::Interpolated(parts) => parts.iter().any(|part| match part {
            FStringPart::Expr(expr) => expr_contains_complex_lambda(expr),
            FStringPart::Text(_) => false,
        }),
    }
