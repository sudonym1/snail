...
use super::constants::{SNAIL_EXCEPTION_VAR, SNAIL_TRY_NO_FALLBACK_HELPER};
    ...
    fn desugar_expr_no_hoist(&mut self, expr: &Expr, prelude: &mut Vec<Stmt>) -> Expr {
        match expr {
            ...
            Expr::TryExpr {
                ...
                if Self::is_hoistable_compound(unwrapped) {
                    // Compound TryExpr: rewrite to Expr::Try so the compound
                    // runs inside the try scope and exceptions are caught.
                    let compound = unwrap_parens_owned((**expr).clone());
                    let exc_name = SNAIL_EXCEPTION_VAR.to_string();
                    ...
                    let handler_body = match fallback {
                        Some(fb) => {
                            let replaced = replace_exception_var(fb, &exc_name);
                            vec![Stmt::Expr {
                                value: replaced,
                                semicolon_terminated: false,
                                span: span.clone(),
                            ...
                            vec![Stmt::Expr {
                                value: Expr::Call {
                                    func: Box::new(Expr::Name {
                                        name: SNAIL_TRY_NO_FALLBACK_HELPER.to_string(),
                                        span: span.clone(),
                                    }),
                    ...
                    // Recurse to desugar the body/handlers of the new Try
                    self.desugar_expr_no_hoist(&try_expr, prelude)
                } else {
                    // Non-compound: desugar normally (keep as TryExpr for lambda lowering)
                    Expr::TryExpr {
                        expr: Box::new(self.desugar_expr(expr, prelude)),
                        fallback: fallback
                            .as_ref()
...
/// Unwrap Paren layers to get to the inner expression (borrowed).
fn unwrap_parens(expr: &Expr) -> &Expr {
    match expr {
        Expr::Paren { expr, .. } => unwrap_parens(expr),
        other => other,
    }
}

/// Unwrap Paren layers (owned).
fn unwrap_parens_owned(expr: Expr) -> Expr {
    match expr {
        Expr::Paren { expr, .. } => unwrap_parens_owned(*expr),
        other => other,
    }
}

/// Replace all `Expr::Name { name: "$e" }` in an expression with `Expr::Name { name }`.
fn replace_exception_var(expr: &Expr, name: &str) -> Expr {
    match expr {
        Expr::Name { name: n, span } if n == "$e" => Expr::Name {
            name: name.to_string(),
            span: span.clone(),
        },
        // Leaf nodes: no $e possible
        Expr::Name { .. }
        | Expr::Placeholder { .. }
        | Expr::Number { .. }
        | Expr::String { .. }
        ...
        | Expr::Regex { .. }
        | Expr::FieldIndex { .. }
        | Expr::StructuredAccessor { .. } => expr.clone(),

        // Recurse into common expression variants
        Expr::Attribute { value, attr, span } => Expr::Attribute {
            value: Box::new(replace_exception_var(value, name)),
            attr: attr.clone(),
            span: span.clone(),
        },
        Expr::Index { value, index, span } => Expr::Index {
            value: Box::new(replace_exception_var(value, name)),
            index: Box::new(replace_exception_var(index, name)),
            span: span.clone(),
        },
        Expr::Call { func, args, span } => Expr::Call {
            func: Box::new(replace_exception_var(func, name)),
            args: args
                .iter()
                .map(|a| replace_exception_var_in_arg(a, name))
                .collect(),
            span: span.clone(),
        },
        Expr::Unary {
            ...
            op: *op,
            expr: Box::new(replace_exception_var(inner, name)),
            span: span.clone(),
        },
        Expr::Binary {
            left,
            ...
            left: Box::new(replace_exception_var(left, name)),
            op: *op,
            right: Box::new(replace_exception_var(right, name)),
            span: span.clone(),
        },
        ...
        Expr::List { elements, span } => Expr::List {
            elements: elements
                .iter()
                .map(|e| replace_exception_var(e, name))
                .collect(),
            span: span.clone(),
        },
        Expr::Tuple { elements, span } => Expr::Tuple {
        ...
        Expr::TryExpr {
            ...
            fallback: fallback
                .as_ref()
                .map(|fb| Box::new(replace_exception_var(fb, name))),
            span: span.clone(),
        },
        // For any other variant, just clone (no $e expected inside)
        other => other.clone(),
    }
}

fn replace_exception_var_in_arg(arg: &Argument, name: &str) -> Argument {
    match arg {
        Argument::Positional { value, span } => Argument::Positional {
            value: replace_exception_var(value, name),
            span: span.clone(),
        },
        Argument::Keyword {
            name: kw,
